...
### 注意事項
* 本工具先將清單中的證書刪除，再將這些證書添加到 CRL 憑證撤銷清單中，在 CRL 憑證撤銷清單中的證書才能被徹底禁用。 **直接將證書直接刪除並沒有任何作用，下次訪問使用該證書的網站時會重新自動聯網添加**
* 由於 Windows 系統的機制，吊銷時使用普通許可權與使用管理員許可權運行批次處理並沒有區別，可隨意選擇。 同時**由於 Windows 系統下每個使用者使用獨立的證書清單，需要所有使用者都運行一次本工具才能徹底禁用證書的使用！**
